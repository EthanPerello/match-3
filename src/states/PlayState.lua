...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        local x, y = mouseX *512/1280 -256, 288/720 * mouseY
        if love.mouse.isDown(1) then
            if (math.floor((x-16)/32)) + 1 <= 8 and (math.floor((x-16)/32)) + 1 >= 0 then
                if (math.floor((y-32-16)/32)) + 1 <= 8 and (math.floor((y-32-16)/32)) + 1 >= 0 then
                    if x % 32 > 0.5 then
                        self.boardHighlightX = (math.floor((x-16)/32)) + 1
                    end
                    if y % 32 > 0.5 then
                        self.boardHighlightY = (math.floor((y-32-16)/32)) + 1
                    end
                    if x % 32 < 0.5 then
                        self.boardHighlightX = (math.floor((x-16)/32)) + 1
                    end
        ...
        if self.mousePressed == true then
            
            -- if same tile as currently highlighted, deselect
            local x = self.boardHighlightX + 1
            local y = self.boardHighlightY + 1

            local revertBoard = self.board
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        self.score = self.score + self.board.scoreAdded
    ...
    if self.board:checkPossibleMatches() == false then
        self.board = nil
